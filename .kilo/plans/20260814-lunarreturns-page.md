...
## Fix: сообщение о сохранении без uid

### Контекст

`cloudPut()` выводит `status("База сохранена в облако (users/{uid}/db.json)")` с
литеральным `{uid}`. Рассматривали возврат uid из функции и извлечение из
presigned URL, но пользователь выбрал более простой и безопасный вариант: не
показывать uid вовсе (персональный идентификатор не должен лишний раз попадать
на экран), а сообщение привести к стилю `cloudGet()`.

### Решение — единственная правка в `LunarReturns/index.html`

`cloudPut()`, строка после успешного PUT:

```js
// было
status("База сохранена в облако (users/{uid}/db.json)");
// стало
status("База сохранена в облако. Записей: " + db.length);
```

Симметрично `cloudGet()`: «База загружена из облака. Записей: N».

Функцию (`/home/koka/src/LunarReturns/function/handler.py`) не трогаем,
передеплой не нужен, контракт `{url, birthday}` не меняется.

### Вне объёма

- Показ uid где-либо на странице (логин, статус) — отклонено пользователем.
- Правки облачной функции.

### Валидация

1. Синтаксис инлайн-скрипта страницы (`new Function(...)`).
2. Ручной тест: сохранить базу — сообщение «База сохранена в облако. Записей: N».
